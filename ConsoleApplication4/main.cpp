...
bool are_brackets_matching(char open, char close) { //дл€ перев≥рки на в≥дпов≥дн≥сть
    if (open == '(' && close == ')')
    {
        return true;
    }
...
void check_brackets(string line) {
    Stack<char> stack;
    for (size_t i = 0; i < line.size(); i++) {
        char current_char = line[i]; //тут розбиваЇмо кожний символ окремо ≥ збер≥гаЇмо в current_char

        if (current_char == '(' || current_char == '{' || current_char == '[') { //€кщо це дужка до пушимо в стек
            ...
            char open_bracket = stack.pop(); //вит€гуЇмо останю дужку на в≥дпов≥дсть через pop

            if (!are_brackets_matching(open_bracket, current_char)) { //перев≥р€Їмо на в≥дпов≥дсть ( )
                cout << "Parentheses do not match" << endl;
            }
        }
